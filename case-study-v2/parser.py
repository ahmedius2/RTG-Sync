...
class Parser:
    ...
    def extract_dnn_job_durations (self, runLog):
        self.__check_file_exists (runLog)

        jobDurations = []
        parsedData = self.__parse_dnn_runlog (runLog)

        for frameNumber in parsedData:
            jobDurations.append (parsedData [frameNumber]['jd'])

        return jobDurations

    def __parse_dnn_runlog (self, fileName):
        lines = []
        parsedData = {}
        regex = '^\s+([\d]+)\|\s+([\d.]+)\|\s+([\d.]+)\|.*$'
        ...
        for line in lines:
            ...
            if m:
                ...
                jobDuration = float (m.group (2))
                responseTime = float (m.group (3))

                parsedData [frameNumber] = {'jd': jobDuration,
                                                 'rt': responseTime}

        return parsedData

    def parse_rtgsynch_trace (self, fileName):
        '''
            Data-Structure
