...
int g_nthreads = 1;
bool g_delay = false;
bool g_npp_block = false;
int g_gang_id = 0;
pthread_barrier_t *g_gang_barrier = NULL;
...
void worker(void *param)
{
	...
	if (g_gang_id != 0) rtg_member_sync(g_gang_barrier);

	for (j = 0;; j++) {
		unsigned int l_start, l_end, l_duration;
		if (g_delay) usleep(10 * 1000);

		l_start = get_usecs();
		if (g_npp_block) npp_lock();
...
int main(int argc, char *argv[])
{
	 ...
	 */
	while ((opt = getopt_long(argc, argv, "m:n:a:t:c:r:p:i:j:l:g:hbdv:o",
		...
		case 'c': /* set CPU affinity */
			...
			break;
		case 'g': /* run as member of virtual gang */
			g_gang_id = strtol(optarg, NULL, 0);
			break;
		...
		case 'l': /* period -> determine P (ms)*/
			...
			break;
		case 'b': /* Execute non-preemptively */
			g_npp_block = true;
			break;
		case 'd':
			g_delay = true;
	...
	if (g_gang_id != 0) g_gang_barrier = rtg_member_setup(g_gang_id, 0, 0, 0);
	g_start = get_usecs();
