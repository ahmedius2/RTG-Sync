#############################################################################
# Experiment Script: Random Gang Selection
#
# This script demonstrates the utility of gang-selection algorithm by
# show-casing the adverse effects of careless gang formation via random member
# selection policy. The gang members are synchronized amongst themselves at
# startup.
#
...
taskset -c ${tracing_core} ${rtg_sync_path}/rtg_daemon &
sleep 1
...
wait $!
${rtg_sync_path}/rtg_client -f ${bw_rt_gang_id}
...
if [ "$2" == "trace-cmd" ]; then
	...
	# Rename trace file
	mv trace.dat trace.grand_${1}
else
	sleep 2
	cp /sys/kernel/debug/tracing/trace ktrace.grand_${1}
	echo > /sys/kernel/debug/tracing/trace
fi
