...
class Parser:
    ...
    def parse (self, fileName):
        ...
        for line in lines:
            ...
            if l:
                ...
                self.__insert_event (rtgid, timeStamp, event)

        if self.debug:
            self.__debug_print_parsed_data_per_gang ()

        return self.parsedData, self.eventData
    ...
    def __insert_event (self, gangId, timeStamp, event):
        if event == 'job_start' and gangId != 1001:
            return

        if timeStamp in self.eventData:
            timeStamp += 0.000001

        self.eventData [timeStamp] = event
